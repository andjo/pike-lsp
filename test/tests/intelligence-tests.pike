...
int main(int argc, array(string) argv) {
    ...
    run_test(test_resolve_stdlib_array, "resolve_stdlib: resolves Array module");
...
void test_resolve_stdlib_module() {
    ...
    if (!result->result->exists) {
        error("Expected Array module to exist in stdlib\n");
    }

    string path = result->result->path;
    if (!path || sizeof(path) == 0) {
        error("Expected non-empty path for Array module\n");
    }
}
...
//! Test resolve stdlib Array module
void test_resolve_stdlib_array() {
    ...
    if (!result->result->found) {
        error("Expected found=1 for Array module\n");
    }

    // Should have symbols
    if (!result->result->symbols || sizeof(result->result->symbols) < 1) {
        error("Expected symbols in Array module result\n");
    }
}
...
void test_resolve_stdlib_with_docs() {
    ...
    if (!result->result || !result->result->found) {
        error("Expected to find Array module\n");
    }
    ...
    error("Expected at least some symbols in Array module\n");
}
