...
DO NOT include cache interaction in this task - Parser is stateless.
DO NOT include error catching at this stage - that will be added in handler wrapper.

Keep all the preprocessing logic (lines 97-129), PikeParser usage (lines 131-386), and error handling (lines 389-405).
...
Note: The old handle_parse body (lines 97-412) is now in Parser.pike, so we can replace the entire function.
  ...
  <done>
    handle_parse delegates to Parser.parse_request
    Old helper functions removed from analyzer.pike
    JSON-RPC errors returned on exceptions
  </done>
</task>
...
6. analyzer.pike handle_parse delegates to Parser class
7. Old helper functions removed from analyzer.pike
8. Full end-to-end test: analyzer.pike responds to parse method with correct output
</verification>
...
5. analyzer.pike handle_parse delegates to Parser class with error catching
6. Old helper functions removed from analyzer.pike
7. JSON-RPC parse method still works correctly after refactoring
</success_criteria>
