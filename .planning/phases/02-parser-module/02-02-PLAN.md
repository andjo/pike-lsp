...
must_haves:
  truths:
    ...
    - "All handlers wrapped in catch blocks returning JSON-RPC errors"
  artifacts:
...
- Keep error handling (catch block) returning JSON-RPC error
...
- Keep error handling for each file (continues even if one file fails)
- No migration needed (no String.trim_whites in this function)
...
```
  ...
  <done>
    ...
    Continues processing even if individual file fails
  </done>
</task>
...
Note: Parser methods have their own error catching, but the handler wrapper provides an additional safety net.
  </action>
  <verify>
  ...
  <done>
    handle_tokenize delegates to Parser.tokenize_request
    handle_compile delegates to Parser.compile_request
    handle_batch_parse delegates to Parser.batch_parse_request
    All handlers wrapped in catch blocks returning JSON-RPC errors
...
5. analyzer.pike handle_tokenize, handle_compile, handle_batch_parse delegate to Parser
6. All handlers have catch blocks returning JSON-RPC errors
7. End-to-end: JSON-RPC requests for tokenize, compile, batch_parse work correctly
</verification>

<success_criteria>
1. Parser class has all four public methods: parse_request, tokenize_request, compile_request, batch_parse_request
2. analyzer.pike delegates all four handlers to Parser class
3. All handlers have error catching returning JSON-RPC error responses
4. JSON-RPC protocol unchanged - client sees same response format
5. No string trimming, cache access, or constant duplication in Parser.pike
</success_criteria>
